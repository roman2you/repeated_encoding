...
/**
 * Write integers or longs with delta encoding and bit packing
 * The format is as follows:
 * <p/>
 * <pre>
 *   {@code
 *     delta--packing: <page-header> <block>*
 ...
 *
 *     min delta : zig-zag varInt encoded so the value max is 32
 *     bitWidthsOfMiniBlock : 1 byte little endian
 *     blockSizeInValues,blockSizeInValues,totalValueCount : unsigned varnt
 *     firstValue : zigzag varInt or varLong
 *   }
 * </pre>
 *
 ...
 *
 */
public class DeltaIntBitPackingValuesWriter extends ValuesWriter {
  /**
   ...
   */
  protected int minDeltaInCurrentBlock = Integer.MAX_VALUE;
   ...
   */
  protected int[] deltaBlockBuffer;


  /**
   * firstValue is written to the header of the page
   */
  protected int firstValue = 0;
  ...
  public DeltaIntBitPackingValuesWriter() {
  }

  public DeltaIntBitPackingValuesWriter(int initSlabSize) {
    this(DEFAULT_NUM_BLOCK_VALUES, DEFAULT_NUM_MINIBLOCKS, initSlabSize);
  }

  public DeltaIntBitPackingValuesWriter(int blockSizeInValues, int miniBlockNum, int initSlabSize) {
    ...
    baos = new CapacityByteArrayOutputStream(initSlabSize);
  }
  ...
  protected boolean flushBlockCondition() {
  ...
  protected void writeBitWidthForMiniBlock(int i) {
    try {
      ...
      throw new ZebraEncodingException("can not write bitwith for miniblock", e);
    }
  }

  protected void writeMinDelta() {
    try {
      BytesUtils.writeZigZagVarInt(minDeltaInCurrentBlock, baos);
    } catch (IOException e) {
      throw new ZebraEncodingException("can not write min delta for block", e);
    }
  }
  ...
  public String memUsageString(String prefix) {
    return String.format("%s DeltaBitPacking %d bytes", prefix, getAllocatedSize());
  }
}
