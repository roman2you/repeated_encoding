...
public class ByteBasedBitPackingEncoder {
  ...
  public BytesInput toBytes() throws IOException {
    ...
    LOG.debug("writing " + (slabs.size() * slabSize + packedByteLength) + " bytes");
    if (inputSize > 0) {
    ...
    return BytesInput.concat(BytesInput.concat(slabs), BytesInput.from(packed, 0, packedByteLength));
  }
  ...
  public static void main(String[] args) throws Exception {
    ByteBasedBitPackingEncoder encoder = new ByteBasedBitPackingEncoder(3, Packer.LITTLE_ENDIAN);
    Random random = new Random();
    random.nextInt();
    for(int i = 0; i < 12; i++) {
      encoder.writeInt(random.nextInt(3));
    }
    encoder.toBytes();
  }
